...
import dev.l2j.tesla.autobots.behaviors.attributes.*
...
import java.util.Random // Necessário para a lógica de fuga

internal abstract class CombatBehavior(val player: Player, var combatPreferences: CombatPreferences) :
    ConsumableUser {
    ...
    internal suspend fun onUpdate() {
        ...
        // Checa se o HP tá abaixo de 30%
        if (player.currentHp < player.maxHp * 0.3 && Random().nextInt(100) < 80) { // 80% de chance de correr
            // Move pra uma direção aleatória pra "fugir"
            val fleeX = player.x + (Random().nextInt(800) - 400) // Move entre -400 e +400 no X (~800 metros)
            val fleeY = player.y + (Random().nextInt(800) - 400) // Move entre -400 e +400 no Y (~800 metros)
            player.ai.setIntention(IntentionType.MOVE_TO, Location(fleeX, fleeY, player.z))

            // Lista de 30 frases aleatórias sem acento pra quando foge
            val fleeQuotes = listOf(
                "To saindo fora HP baixo!",
                "Preciso correr agora!",
                "Vou dar no pe HP ta critico!",
                "Nao da mais to fugindo!",
                "HP no chao vou escapar!",
                "To correndo pra sobreviver!",
                "Me segura que eu to fora!",
                "Vou me mandar HP acabou!",
                "Nao aguento mais to correndo!",
                "Foge que ta feio!",
                "HP baixo demais ja era!",
                "To pulando fora disso!",
                "Vou correr antes que morra!",
                "Nao da pra ficar to saindo!",
                "Socorro HP no fim!",
                "Corre que ta perigoso!",
                "Vou me esconder rapidinho!",
                "To fora antes que piore!",
                "HP quase zero corro ja!",
                "Nao vou morrer to fugindo!",
                "Vou dar um tempo HP baixo!",
                "To correndo pro respawn!",
                "Nao aguento to me mandando!",
                "Foge que o bicho pega!",
                "HP critico to saindo!",
                "Vou correr pra me curar!",
                "To fora HP nao da mais!",
                "Corre antes que me peguem!",
                "Vou me salvar to correndo!",
                "Nao da pra lutar to fugindo!"
            )

            // Escolhe uma frase aleatória da lista
            val randomFleeQuote = fleeQuotes[Random().nextInt(fleeQuotes.size)]
            (player as? Autobot)?.say(randomFleeQuote) // Cast seguro pra Autobot

            delay(10000L) // Corre por 10 segundos antes de continuar
            return // Sai do método pra não atacar enquanto foge
        }
    ...
    override val consumables: List<Consumable> = mutableListOf(
        ...
        Consumable(1539) { autobot ->
            autobot.getCombatBehavior().combatPreferences.useGreaterHealingPots && autobot.getHealthPercentage() < 80 && !autobot.hasEffect(
                2037
            )
        },
