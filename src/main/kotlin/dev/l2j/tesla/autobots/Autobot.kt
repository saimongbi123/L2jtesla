...
class Autobot(objectId: Int, template: PlayerTemplate?, app: Appearance?) : Player(objectId, template, "Autobots", app) {
    ...
    override fun doDie(killer: Creature?): Boolean {
        val died = super.doDie(killer) // Chama o método original pra garantir que a morte aconteça normalmente

        if (died) { // Só fala se realmente morreu
            // Lista de frases aleatórias que o bot pode dizer ao morrer
            val deathQuotes = listOf(
                "Nao acredito que cai dessa vez!",
                "Foi por pouco ou nao!",
                "Hora de um respawn estrategico!",
                "Meus calculos falharam miseravelmente!",
                "Que golpe baixo hein!",
                "Vida de bot nao e facil...",
                "Fui pro chao mas levanto ja!",
                "Isso merece uma revanche!",
                "Um dia eu acerto essa defesa!",
                "Poxa justo agora?",
                "Derrotado mas nao acabado!",
                "Alguem anotou a placa desse golpe?",
                "Ta bom voces venceram por enquanto!",
                "Minha bateria acabou socorro!",
                "Esse foi um bug fatal!",
                "Nao era pra ser assim!",
                "A culpa e do lag certeza!",
                "Volto com upgrades aguardem!",
                "Fui pego desprevenido dessa vez!",
                "Morte temporaria ja ja to de volta!",
                "Isso foi so um teste serio!",
                "Preciso de um patch urgente!",
                "Que humilhacao hein!",
                "Fui pro offline mas ja volto!",
                "Esse killer e cheatado so pode!",
                "Minha IA falhou feio agora!",
                "Um passo atras dois a frente!",
                "Adeus mundo digital!",
                "Isso vai pro meu log de erros!",
                "Me derrubaram mas eu me levanto!"
            )

            // Escolhe uma frase aleatória
            val randomQuote = deathQuotes[Random().nextInt(deathQuotes.size)]

            // Faz o bot falar a frase
            say(randomQuote)
        }
    ...
    // Função pra aceitar convite de party
    fun acceptPartyInvite(requestor: Player?) {
        if (requestor == null) {
            println("Requestor é null ao tentar aceitar convite de party para ${this.name}")
            return
        }
        GlobalScope.launch {
            delay(2000L) // 2 segundos de atraso pra simular resposta
            joinParty(requestor)
        }
    }

    // Lógica de entrar na party
    private fun joinParty(requestor: Player) {
        ...
        if (party != null) {
            ...
            party.addPartyMember(this) // Adiciona o Autobot à party existente
            // Força atualização da UI pra todos os membros
            party.broadcastToPartyMembers(PartySmallWindowAll(null, party))
        } else {
            // Cria uma nova party se o requestor não tiver uma
            val lootRule = requestor.lootRule ?: LootRule.ITEM_LOOTER // Valor padrão se lootRule for null
            val newParty = Party(requestor, this, lootRule)
            // Força atualização da UI pra todos os membros
            newParty.broadcastToPartyMembers(PartySmallWindowAll(null, newParty))
        }

        // Feedback pro jogador real
        requestor.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_JOINED_PARTY).addCharName(this))

        // Feedback pro Autobot
        sendMessage("Entrei na party de ${requestor.name}!")

        // Atualiza a UI de todos na party
        broadcastUserInfo()
    }
}

private fun Party.broadcastToPartyMembers(all: dev.l2j.tesla.gameserver.network.serverpackets.PartySmallWindowAll) {}