...
import dev.l2j.tesla.autobots.models.CreateBotDetails
import dev.l2j.tesla.autobots.ui.UiComponents.checkboxComponent
import dev.l2j.tesla.autobots.ui.UiComponents.comboboxComponent
import dev.l2j.tesla.autobots.ui.UiComponents.textbotComponent
import dev.l2j.tesla.autobots.ui.html.HtmlAlignment
import dev.l2j.tesla.autobots.ui.states.*
import dev.l2j.tesla.autobots.ui.tabs.*
import dev.l2j.tesla.autobots.utils.IconsTable
import dev.l2j.tesla.autobots.utils.getSupportedClassesForLevel
    ...
    private fun sendWindow(player: Player, html: String){
        ...
        when {
            ...
            optimisedHtml.length < 8180 -> {
                player.sendPacket(
                    ShowBoard(
                        optimisedHtml.substring(0, 4090),
                        "101"
                    )
                )
                player.sendPacket(
                    ShowBoard(
                        optimisedHtml.substring(4090, optimisedHtml.length),
                        "102"
                    )
                )
                player.sendPacket(ShowBoard.STATIC_SHOWBOARD_103)
            ...
            optimisedHtml.length < 12270 -> {
                ...
                player.sendPacket(
                    ShowBoard(
                        optimisedHtml.substring(4090, 8180),
                        "102"
                    )
                )
                player.sendPacket(
                    ShowBoard(
                        optimisedHtml.substring(8180, optimisedHtml.length),
                        "103"
                    )
                )
            }
