		...
		else if (command.startsWith("admin_setname"))
		{
			try
			{
				...
				if (target instanceof Player)
				{
					...
					// Name is a npc name.
					// Name already exists.
					if (!StringUtil.isValidString(newName, "^[A-Za-z0-9]{3,16}$") || (NpcData.getInstance().getTemplateByName(newName) != null) || (PlayerInfoTable.getInstance().getPlayerObjectId(newName) > 0))
					{
