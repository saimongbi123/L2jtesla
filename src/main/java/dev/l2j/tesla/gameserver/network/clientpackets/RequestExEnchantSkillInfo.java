...
public final class RequestExEnchantSkillInfo extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if ((player == null) || player.getClassId().level() < 3 || player.getLevel() < 76)
			return;

		final Folk folk = player.getCurrentFolk();
		if (folk == null || !folk.canInteract(player) || (player.getSkillLevel(_skillId) >= _skillLevel))
			return;

		final L2Skill skill = SkillTable.getInstance().getInfo(_skillId, _skillLevel);
		if ((skill == null) || !folk.getTemplate().canTeach(player.getClassId()))
			return;
