...
public class AttackableAI extends CreatureAI implements Runnable
{
	...
	protected boolean autoAttackCondition(Creature target)
	{
		...
		if (target instanceof Playable)
		{
			...
			if (!me.isInsideRadius(target, me.getTemplate().getAggroRange(), true, false) || (!(me.isRaidRelated()) && !(me.canSeeThroughSilentMove()) && ((Playable) target).isSilentMoving()))
				return false;
		...
		if (willCastASpell())
		{
			...
			if (!defaultList.isEmpty())
			{
				...
				if (master != null && !master.isDead() && (master.getCurrentHp() / master.getMaxHp() < 0.75))
				{
					...
					{
						if ((sk.getTargetType() == L2Skill.SkillTargetType.TARGET_SELF) || !checkSkillCastConditions(sk))
							continue;
