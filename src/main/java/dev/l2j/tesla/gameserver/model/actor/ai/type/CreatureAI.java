...
public class CreatureAI extends AbstractAI
{
	...
	{
		if ((target == null) || (_desire.getIntention() == IntentionType.REST))
		{
	...
	{
		if ((_desire.getIntention() == IntentionType.REST) || _actor.isAllSkillsDisabled() || _actor.isCastingNow())
		{
			// Cancel action client side by sending Server->Client packet ActionFailed to the Player actor
			clientActionFailed();
			return;
	...
	{
		if ((_desire.getIntention() == IntentionType.REST) || _actor.isAllSkillsDisabled() || _actor.isCastingNow() || _actor.isMovementDisabled())
		{
			// Cancel action client side by sending Server->Client packet ActionFailed to the Player actor
			clientActionFailed();
			return;
	...
	protected void onIntentionPickUp(WorldObject object)
	{
		...
		if ((_desire.getIntention() == IntentionType.REST) || _actor.isAllSkillsDisabled() || _actor.isCastingNow() || _actor.isAttackingNow())
		{
			clientActionFailed();
			return;
		}
